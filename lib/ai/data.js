...
export async function findCompletedQuizzesByUserInRange(userId, start, end) {
  const database = await getDatabase();
  return database.collection("quizSessions")
    .find(
      { userId, completedAt: { $gte: start, $lt: end } },
      { projection: { category: 1, difficulty: 1, score: 1, completedAt: 1 } }
    )
    .sort({ completedAt: 1 })
    .toArray();
}
