...
const QUIZ_CATEGORIES = ["전체", ...WORD_CATEGORIES];
...
export { QUIZ_DIFFICULTIES, QUIZ_CATEGORIES, QUIZ_QUESTION_COUNT };
...
export function validateAiQuizOutput(value, wordIds, difficulty) {
  ...
  if (value.questions.length !== QUIZ_QUESTION_COUNT) {
    return { quiz: null, error: "AI_QUIZ_INVALID" };
  }
...
export function validateQuizAnswers(formData) {
  const values = Array.from({ length: QUIZ_QUESTION_COUNT }, (_, index) => formData.get(`answer-${index}`));
  if (values.some((value) => typeof value !== "string" || !/^[0-3]$/.test(value))) {
    return { answers: null, error: "세 문제의 답을 모두 선택해 주세요." };
  }
